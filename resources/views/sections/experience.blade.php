                ...
                <div class="md:grid md:grid-cols-5 md:gap-8 items-start">
                    ...
                    <div class="md:col-span-4 bg-slate-800 rounded-2xl p-8 border border-slate-700 shadow-lg relative">
                        ...
                        <div class="flex items-center gap-2 mb-4">
                            <span class="px-2 py-1 bg-{{ $exp['color'] }}-500/20 text-{{ $exp['color'] }}-400 text-xs rounded font-semibold">{{ __($exp['type_key']) }}</span>
                            @if(!empty($exp['location_type']))
                                <span class="px-2.5 py-1 bg-slate-900/90 text-slate-300 text-xs rounded-full border border-slate-700/60 font-medium inline-flex items-center gap-1">
                                    <i class="fa-solid fa-house-laptop text-cyan-400"></i> {{ $exp['location_type'] }}
                                </span>
                            @endif
                            @if(!empty($exp['linkedin']))
                                <a href="{{ $exp['linkedin'] }}" target="_blank" class="text-slate-400 hover:text-white transition-colors ml-auto" aria-label="LinkedIn">
